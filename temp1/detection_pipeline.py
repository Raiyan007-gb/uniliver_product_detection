...
class YOLOv8DetectionPipeline:
    ...
    def process_image(self, image_path=None, output_path=None, visualize=True, image=None):
        ...
        Args:
            ...
            image (numpy.ndarray, optional): Direct image array input
        ...
        """
        # Read the image from path or use provided image
    ...
    def process_directory(self, input_dir, output_dir=None, visualize=True, save_results=True):
        ...
        class_confidences = {}  # Track confidence scores per class
        ...
        for img_path in tqdm(image_files, desc="Processing images"):
            ...
            # Update total counts and class confidences
            total_counts.update(class_counts)
            
            # Update class confidences
        ...
        if output_dir and save_results:
            with open(os.path.join(output_dir, "detection_summary.txt"), 'w') as f:
                ...
                f.write("\n===== TOTAL OBJECT COUNTS AND ACCURACIES ACROSS ALL IMAGES =====\n")
                for class_name, count in total_counts.items():
                    avg_class_conf = sum(class_confidences[class_name]) / len(class_confidences[class_name])
                    class_accuracy = self.get_accuracy_percentage(avg_class_conf)
                    f.write(f"{class_name}: {count} (Accuracy: {class_accuracy:.1f}%)\n")
